...
public sealed class Microsoft365HtmlContentExtractorAdapter(
    MicrosoftHtmlContentExtractorClient client,
    IOptions<Microsoft365Options> options) : IMicrosoft365ContentExtractor
{
    private static readonly HashSet<string> Extensions = new(StringComparer.OrdinalIgnoreCase) { ".html", ".htm" };
    ...
    private static Microsoft365ExtractedContentUnit MapUnit(MicrosoftTextBasedExtractedUnit unit) =>
        new(
            unit.IsTitle ? Microsoft365ExtractedContentUnitKind.Title : Microsoft365ExtractedContentUnitKind.Paragraph,
            unit.Order,
            unit.Text,
            unit.SourcePart);
}
