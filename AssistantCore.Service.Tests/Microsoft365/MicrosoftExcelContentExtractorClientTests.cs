...
public sealed class MicrosoftExcelContentExtractorClientTests
{
    ...
    [Theory, AutoDomainData]
    public async Task Given_CsvRowsStoredInSingleCells_When_ReadAsync_Then_ReturnsEveryStructuredRow(
        string fileName)
    {
        // Given
        await using var package = CreateWorkbook(
            ...
            new Dictionary<string, string>
            {
                ["xl/worksheets/sheet1.xml"] =
                    "<worksheet xmlns=\"http://schemas.openxmlformats.org/spreadsheetml/2006/main\"><sheetData>"
                    + "<row r=\"1\"><c r=\"A1\" t=\"inlineStr\"><is><t>Transaction ID,Account Name,Transaction Amount</t></is></c></row>"
                    + "<row r=\"2\"><c r=\"A2\" t=\"inlineStr\"><is><t>159,&quot;North, Inc.&quot;,3100.50</t></is></c></row>"
                    + "<row r=\"3\"><c r=\"A3\" t=\"inlineStr\"><is><t>308,South,4200.75</t></is></c></row>"
                    + "</sheetData></worksheet>",
                ["xl/_rels/workbook.xml.rels"] =
                    "<Relationships xmlns=\"http://schemas.openxmlformats.org/package/2006/relationships\"><Relationship Id=\"rId1\" Target=\"worksheets/sheet1.xml\"/></Relationships>"
            });
        ...
        var worksheet = Assert.Single(workbook.Worksheets);
        Assert.Equal(2, worksheet.Rows.Count);
        Assert.Equal("North, Inc.", worksheet.Rows[0]["Account Name"]);
        Assert.Equal("4200.75", worksheet.Rows[1]["Transaction Amount"]);
    }
