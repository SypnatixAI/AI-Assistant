...
internal sealed class EvaluationReportWriter
{
    ...
    internal static string CreateMarkdown(EvaluationReport report)
    {
        ...
        var builder = new StringBuilder()
            ...
            .AppendLine("## Summary")
            .AppendLine()
            .AppendLine("| Status | Mode | Agent | Passed | Failed | Duration | ACL leaks |")
            .AppendLine("|---|---|---|---:|---:|---:|---:|")
            .AppendLine($"| {statusIcon} {statusLabel} | `{EscapeCell(report.Mode)}` | `{EscapeCell(report.AgentIdentifier)}` | {report.Summary.Passed} | {report.Summary.Failed} | {FormatNumber(report.Summary.DurationMilliseconds)} ms | {report.Summary.AclLeakageCount} |")
            .AppendLine()
