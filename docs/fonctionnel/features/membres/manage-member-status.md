...
L'appelant doit être un membre interne actif avec le rôle `Admin`. Le membre
cible doit appartenir à la même organisation.
...
Alice est Admin active. Bob est User actif de la même organisation. Alice
...
Alice et Marc sont les deux derniers Admin actifs. Deux requêtes simultanées
tentent de les désactiver. La transaction et la protection de concurrence
doivent permettre au maximum une désactivation. L’autre requête retourne :
...
```json
{
  ...
  "message": "L’organisation doit conserver au moins un administrateur actif."
}
```
...
1. Valider le JWT, l'organisation et l'Admin courant.
...
5. Lors d'une désactivation d'Admin, compter les autres Admin actifs dans la
   même transaction.
6. Enregistrer le statut avec protection contre les modifications concurrentes.
7. Ajouter l'entrée d'audit dans la même unité cohérente.
8. Retourner le membre actualisé.
...
- `400` : identifiant/statut invalide, modification de soi-même ou dernier Admin.
- `401` : token absent ou invalide.
- `403` : appelant non Admin, membre ou organisation inactive.
- `404` : cible absente ou appartenant à une autre organisation.
- `409` : modification concurrente empêchant de garantir le dernier Admin.
...
- Un Admin peut désactiver et réactiver un autre membre de son organisation.
- Un membre désactivé est immédiatement refusé par les endpoints protégés.
- Le rôle du membre ne change pas.
- Le dernier Admin actif reste protégé, y compris avec deux requêtes concurrentes.
